...
** Special indications for running on LUMI

  You will need to run the code compiled in my home directory:
  
  #+begin_src bash
source /users/scemamaa/qp2/quantum_package.rc
  #+end_src

  On the login nodes of LUMI, it is impossible to run the code
  interactively, as you would do on your laptop. To run interactively,
  you need to request an interactive job. Here is a command to request
  a compute node for one hour:

  #+begin_src bash
salloc -N 1 --account=project_465000321 --time=01:00:00 --exclusive -n 1 -c 64 --partition=small
  #+end_src
  
  Then, instead of using the ~qp_run~ command as you would do on your
  laptop, you will need to run ~qp_srun~ instead.
