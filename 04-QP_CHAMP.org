...
jastrow_parameter   1
 ...
 The folder also contains the submission script which is also presented in [[./01-JobScripts.org][section 1]]. submit
 the job using sbatch =lumi_example01.sh=. The output will be stored in =vmc_h2o_hf.out= and you can grep the total energy as
